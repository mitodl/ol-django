"""google sheets settings"""  # noqa: INP001
...
MITOL_GOOGLE_SHEETS_ENROLLMENT_CHANGE_SHEET_ID = get_string(
    ...
    default=None,
    description=(
        "ID of the Google Sheet that contains the enrollment change request worksheets (refunds, transfers, etc)"  # noqa: E501
    ),
)
...
MITOL_GOOGLE_SHEETS_DATE_FORMAT = get_string(
    ...
    default="%m/%d/%Y",
    description="Python strptime format for date columns (no time) in enrollment management spreadsheets",  # noqa: E501
)
_sheets_date_timezone = get_string(
    name="MITOL_GOOGLE_SHEETS_DATE_TIMEZONE",
    default="UTC",
    description=(
        "The name of the timezone that should be assumed for date/time values in spreadsheets. "  # noqa: E501
...
MITOL_GOOGLE_SHEETS_ADMIN_EMAILS = get_list_literal(
    ...
    default=[],
    description="This list of email addresses for the google service account authentication",  # noqa: E501
)
